...
function KCM.Debug.Toggle()
    ...
    local nextValue = not KCM.db.profile.debug
    ...
    if H and H.SetAndRefresh and H.SetAndRefresh("debug", nextValue) then
        KCM.Debug.Print("Debug prints are now enabled.")
        return
    end
    KCM.db.profile.debug = nextValue
    local state = nextValue and "|cff00ff00ON|r" or "|cffff5555OFF|r"
