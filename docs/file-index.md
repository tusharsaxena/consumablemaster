...
| `SpecHelper.lua` | Class/spec identity. `GetCurrent()` returns `(classID, specID, key, specName, className, classFile)`. `GetStatPriority(specKey)` merges user override → seed default → class fallback. `MakeKey(classID, specID)` produces the canonical `<classID>_<specID>` string. |
