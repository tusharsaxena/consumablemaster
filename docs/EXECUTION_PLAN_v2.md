...
- ⬜ M9.31 **M-9 gate.** Respec into a build that grants a known spell listed in the Food seed list (e.g. Recuperate for Rogue). After the spell is learned, `KCM_FOOD` should recompute to the spell pick within one frame — no bag event or spec change required.
- ⬜ M9.32 **M-12 gate.** Force three `EditMacro` failures on a single queued entry (via a debug harness that short-circuits `EditMacro` to return 0); confirm the fourth regen exits the pending entry and prints the give-up notice.
