...
| Version | Changes |
|---------|---------|
| 1.4.0 | **Added**: master enable toggle (`/cm enable`, Settings → General); schema-driven `/cm list / get / set` CLI; About landing page on the top-level Settings panel; comprehensive smoke-test playbook in `docs/smoke-tests.md`.<br>**Changed**: Settings UI migrated to a KickCD-style canvas with sub-categorised pages and an in-page breadcrumb chevron; slash UX rebranded to `/cm` with a cyan `[CM]` chat prefix; viewed spec in Stat Priority now auto-tracks the active spec until the user pins one; AIO and category row layouts retuned for consistent right-edge spacing.<br>**Fixed**: panel rows still hydrate `[Loading]` entries while the addon is disabled; `/cm set` reports DB-not-ready instead of silently failing; secondary stat list dedupes input; combat-time panel opens are blocked at OnShow (covers ESC → AddOns sidebar). |
| 1.3.0 | Two new combo macros — `KCM_HP_AIO` and `KCM_MP_AIO` — that switch between sub-category picks based on combat state. `KCM_HP_AIO` cycles Healthstone → Healing Potion in combat and fires the Food pick out of combat; `KCM_MP_AIO` does the equivalent for Mana Potion / Drink. New **AIO Health** / **AIO Mana** settings pages with toggle and reorder per sub-category. Empty-side fallback prints a chat notice. `/cm dump pick hp_aio` / `mp_aio` show the configured order and assembled body. |
| 1.2.1 | Fixed Lua error on login by switching from the removed `LEARNED_SPELL_IN_TAB` event to its modern replacement; spell-name hydration on level-up still works without a reload. |
| 1.2.0 | Action-bar icon fix: the picked item's icon finally renders on ElvUI, Bartender, and any other action-bar addon. New **`/cm rewritemacros`** command (also Settings → General → Force rewrite macros) for unconditional rewrites. Drag-icon widget on category pages now shows the picked item / spell texture directly. |
| 1.1.0 | Correctness fixes (locked items no longer flap macros, one bad scorer can't break the others, oversized bodies fall back gracefully, combat-deferral retries are bounded). Per-recompute score cache. Discovery GC sweeps items unseen in bags for 30 days. Newly-learned spell entries hydrate without a reload. Category tab reorder. Empty-state cooking-pot fallback icon. |
